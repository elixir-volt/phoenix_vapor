defmodule LiveVueNext.Reactive do
  @moduledoc """
  ...
  ## Usage
      ...
      defmodule MyAppWeb.CounterLive do
        ...
        use LiveVueNext.Reactive, file: "lib/my_app_web/live/Counter.vue"
      end
  ...
  defmacro __using__(opts) do
    ...
    {refs, computeds, functions, props} =
      if script_content do
        LiveVueNext.ScriptSetup.parse(script_content)
      else
        {%{}, %{}, [], []}
      end
      ...
      def mount(params, _session, socket) do
          ...
          |> Enum.reduce(%{}, fn {name, init_expr}, acc ->
            value =
              case init_expr do
                ...
                "[]" -> []
                "{}" -> %{}
                expr -> LiveVueNext.JsEval.eval(expr, %{}) || 0
              end
        ...
        computed_assigns =
          ...
          |> Enum.reduce(%{}, fn {name, expr}, acc ->
            value = LiveVueNext.Expr.eval(expr, socket.assigns)
    ...
    Enum.map(functions, fn func_name ->
      ...
      quote do
        def handle_event(unquote(func_name), params, socket) do
          ...
          new_state = LiveVueNext.Reactive.eval_handler(unquote(body), current, params)
